...
object SYNEREOMain extends js.JSApp {
  ...
  val routerConfig = RouterConfigDsl[Loc].buildConfig { dsl =>
    ...
    (staticRoute(root, SynereoLoc) ~> renderR(ctl => Login(Login.Props()))
      ...
      | staticRoute("#fullpost", PostFullViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.POSTFULL_VIEW)))
      | staticRoute("#userprofile", UserProfileViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.USERPROFILE_VIEW)))
      | staticRoute("#timeline", TimelineViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.TIMELINE_VIEW)))
      | staticRoute("#notifications", NotificationsLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.NOTIFICATIONS_VIEW)))
      | staticRoute("#marketplace", MarketPlaceLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.MARKETPLACE_VIEW))))
