...
object SYNEREOMain extends js.JSApp {
  ...
  val routerConfig = RouterConfigDsl[Loc].buildConfig { dsl =>
    ...
    (staticRoute(root, SynereoLoc) ~> renderR(ctl => Login(Login.Props()))
      ...
      | staticRoute("#postfullview", PostFullViewLOC) ~> renderR(ctl => appProxy(proxy => AppModule(AppModule.Props(AppModule.POSTFULLVIEW_VIEW, proxy))))
      | staticRoute("#userprofileview", UserProfileViewLOC) ~> renderR(ctl => appProxy(proxy => AppModule(AppModule.Props(AppModule.USERPROFILEVIEW_VIEW, proxy))))
      | staticRoute("#timelineview", TimelineViewLOC) ~> renderR(ctl => appProxy(proxy => AppModule(AppModule.Props(AppModule.TIMELINEVIEW_VIEW, proxy))))
      ...
      | staticRoute("#marketplacefull", MarketPlaceLOC) ~> renderR(ctl => appProxy(proxy => AppModule(AppModule.Props(AppModule.MARKETPLACE_VIEW, proxy)))))
