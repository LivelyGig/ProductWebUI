...
object SYNEREOMain extends js.JSApp {
  ...
  val routerConfig = RouterConfigDsl[Loc].buildConfig { dsl =>
    ...
    (staticRoute(root, SynereoLoc) ~> renderR(ctl => Login(Login.Props()))
      ...
      | staticRoute("#people", PeopleLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.PEOPLE_VIEW)))
      | staticRoute("#account", AccountLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.ACCOUNT_VIEW)))
      | staticRoute("#dashboard", DashboardLoc) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.DASHBOARD_VIEW)))
      | staticRoute("#postfullview", PostFullViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.POSTFULLVIEW_VIEW)))
      | staticRoute("#userprofileview", UserProfileViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.USERPROFILEVIEW_VIEW)))
      | staticRoute("#timelineview", TimelineViewLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.TIMELINEVIEW_VIEW)))
      | staticRoute("#notifications", NotificationsLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.NOTIFICATIONS_VIEW)))
      | staticRoute("#marketplacefull", MarketPlaceLOC) ~> renderR(ctl => AppModule(AppModule.Props(AppModule.MARKETPLACE_VIEW))))
